...
class SmartGPUDetector:
    ...
    def _detect_cuda(self):
        ...
        try:
            ...
            if cuda_info:
                ...
                self.logger.info(f"✅ CUDA GPU detected: {len(cuda_info)} device(s)")
                return
            ...
            if cuda_info:
                ...
                self.capabilities.total_vram_mb = sum(gpu.memory_mb for gpu in cuda_info)
                self.capabilities.recommended_layers = self._calculate_cuda_layers(self.capabilities.total_vram_mb)
                self.logger.info(f"✅ CUDA GPU detected via nvidia-smi: {len(cuda_info)} device(s)")
                return
            ...
            if version:
                ...
                if major > 10 or (major == 10 and minor >= 11):
                    ...
                    if metal_info:
                        ...
                        self.logger.info(f"✅ Metal GPU detected: {len(metal_info)} device(s)")
                        return
    ...
    def _log_detection_results(self):
        ...
        if self.capabilities.has_gpu:
            ...
            self.logger.info(f"🔧 Recommended GPU Layers: {self.capabilities.recommended_layers}")
            ...
            self.logger.info("🖥️ Using CPU acceleration (no compatible GPU found)")
