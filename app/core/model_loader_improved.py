"""Model loading functionality with improved tokenization"""
from PySide6.QtCore import QThread, Signal
from typing import Optional
from app.core.config_manager import DEFAULT_CONTEXT_SIZES, DEFAULT_CONTEXT_INDEX
...
class ModelLoader(QThread):
    ...
    def run(self):
        try:
            ...
            # Configure GPU layers - simplified approach that fixes tokenization
            n_gpu_layers = 20 if self.use_gpu else 0
            
            self.progress.emit(f"Initializing {'GPU' if self.use_gpu else 'CPU'} mode...")

            # Load the model with minimal parameters - this fixes tokenization issues
            model = Llama(
