...
from app.core.monitoring import GPUMonitor
    ...
    def _update_model_gpu_status(self, gpu_metrics: List[Dict[str, Any]]):
        ...
        if not gpu_metrics:
            ...
            if avg_gpu_usage > 5:  # Threshold for "in use"
                self.model_gpu_label.setText(
                    f"✅ Hardware Detection: {len(gpu_metrics)} GPU(s) detected\n"
                    f"🚀 Current Usage: GPU is actively being used (avg {avg_gpu_usage:.1f}% utilization)\n\n"
