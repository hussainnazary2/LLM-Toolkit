...
from app.ui.summarization_tab import SummarizationTab
from app.ui.email_automation_tab import EmailAutomationTab
from app.ui.bulk_email_marketing_tab import BulkEmailMarketingTab
from app.ui.social_media_marketing_tab import SocialMediaMarketingTab
from app.ui.about_dialog import AboutDialog
from app.ui.model_loading_dialog import ModelLoadingDialog
from app.ui.system_prompts_dialog import SystemPromptsDialog
from app.ui.model_parameters_dialog import ModelParametersDialog
from app.ui.email_settings_dialog import EmailSettingsDialog
...
from app.services.universal_model_loader import UniversalModelLoader, UniversalLoadingResult
from app.core.universal_events import UniversalLoadingProgress
from app.ui.universal_model_dialog import UniversalModelDialog
from app.core.universal_loading_monitor import UniversalLoadingMonitor
from app.core.universal_format_detector import ModelFormat

# Import resource monitoring components
from app.ui.resource_monitor_header import ResourceMonitorHeader
from app.services.system_resource_monitor import SystemResourceMonitor
...
class MainWindow(QMainWindow):
    ...
    def _log_backend_manager_status(self):
        ...
        try:
            ...
            if not hasattr(self, 'backend_manager') or self.backend_manager is None:
                self.logger.error("❌ BackendManager not initialized - chat operations will fail")
                return
            ...
            self.logger.info("🔧 BackendManager Status Check:")
            self.logger.info(f"   ✅ BackendManager initialized: {type(self.backend_manager).__name__}")
            self.logger.info(f"   📋 Available backends: {', '.join(available_backends) if available_backends else 'None'}")
            ...
            if hasattr(self.backend_manager, 'hardware_detector'):
                ...
                self.logger.info(f"   🖥️  Hardware detected: {hardware_info.gpu_count} GPUs, {hardware_info.total_ram}MB RAM")
                if hardware_info.gpu_count > 0:
                    self.logger.info(f"   🚀 GPU acceleration available: {hardware_info.total_vram}MB VRAM")
            ...
            if self.backend_manager.current_backend:
                ...
                self.logger.info(f"   ✅ Model loaded: {backend_name}")
                self.logger.info("   🎯 BackendManager ready for chat operations")
            else:
                self.logger.info("   ⏳ No model loaded - load a model to enable chat operations")
            ...
            self.logger.error(f"❌ Error checking BackendManager status: {e}")
    ...
    def _load_model_optimized(self, file_path):
        ...
        try:
            ...
            if result.success:
                ...
                self.logger.info(f"✅ Model loaded with optimization:")
                ...
                self.status_bar.showMessage(f"✅ Model loaded: {model_name} ({result.backend_used})")
                ...
                self.status_bar.showMessage(f"❌ Failed to load model: {model_name}")
            ...
            # Update status with error
            self.status_bar.showMessage(f"❌ Error loading model: {model_name}")
    ...
    def log_backend_operation(self, operation: str, success: bool, details: Dict[str, Any] = None):
        ...
        if success:
            self.logger.info(f"✅ Backend operation '{operation}' succeeded: {log_data}")
        else:
            self.logger.error(f"❌ Backend operation '{operation}' failed: {log_data}")
    ...
    def _start_async_text_generation(self, message: str, config):
        ...
        self.logger.info("🚀 Started async text generation with 5-minute timeout to prevent UI freezing")
    ...
    def _on_generation_finished(self, response: str):
        ...
        try:
            ...
            self.logger.info(f"✅ Chat generation completed successfully ({len(response)} chars)")
    ...
    def _on_generation_error(self, error_message: str):
        ...
        try:
            ...
            self.event_bus.publish("chat.error", error_info)
            self.logger.error(f"❌ Chat generation failed: {error_message}")
    ...
    def _on_file_summary_generation_request(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if not hasattr(self, '_pdf_reader'):
                from app.core.threaded_pdf_reader import ThreadedPDFReader
                self._pdf_reader = ThreadedPDFReader()
            
            if not hasattr(self, '_chunked_summarizer'):
                from app.core.chunked_summarization import ChunkedSummarizationManager
    ...
    def _on_backend_manager_backend_changed(self, old_backend: str, new_backend: str):
        ...
        if new_backend:
            self.logger.info("✅ BackendManager ready for chat operations with new backend")
        else:
            self.logger.info("⚠️  No backend loaded - chat operations disabled")
    ...
    def _on_backend_manager_fallback_triggered(self, failed_backend: str, fallback_backend: str, reason: str):
        ...
        if fallback_backend:
            self.logger.info("✅ Chat operations continue with fallback backend")
        else:
            self.logger.warning("⚠️  No fallback available - chat operations may fail")
    ...
    def _on_backend_manager_loading_progress(self, message: str, percentage: int):
        ...
        if percentage < 100:
            ...
            self.logger.info("✅ BackendManager model loading completed - chat operations ready")
    ...
    def _handle_oauth_authentication(self, credentials_path: str):
        ...
        try:
            ...
            if success:
                ...
                from app.ui.force_enable_save_button import force_enable_save_button
    ...
    def _on_universal_loading_completed(self, result: UniversalLoadingResult):
        ...
        if result.success:
            ...
            self.logger.info(f"✅ Universal loading completed successfully:")
            ...
            try:
                ...
                if backend_result and hasattr(backend_result, 'success') and backend_result.success:
                    self.logger.info(f"✅ Model successfully loaded into BackendManager for chat")
                    
                    # Verify the model is actually loaded
                    if self.backend_manager.current_backend:
                        self.logger.info(f"🎯 Chat functionality is now ready with {backend_name}")
                    else:
                        self.logger.warning(f"⚠️ BackendManager reports no current backend after loading")
                        
                elif backend_result:
                    self.logger.warning(f"⚠️ BackendManager loading failed: {backend_result}")
                else:
                    self.logger.warning(f"⚠️ BackendManager returned no result")
                    
            except Exception as e:
                self.logger.error(f"❌ Error bridging to BackendManager: {e}")
                import traceback
    ...
    def _on_system_health_updated(self, health):
        ...
        if health.cpu_usage > 90 or health.memory_usage > 95:
            self.status_bar.showMessage(
                f"⚠️ High resource usage: CPU {health.cpu_usage:.0f}%, Memory {health.memory_usage:.0f}%",
