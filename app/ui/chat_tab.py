...
from app.core.universal_events import (
...
class ChatTab(QWidget):
    ...
    def _show_enhanced_welcome_message(self):
        ...
        welcome_lines = [
            ...
            f"⚡ Performance: {self.current_model_info.performance_summary}",
        ...
        if self.current_model_info.optimization_applied:
            ...
            welcome_lines.append(f"🔧 Optimizations: {optimizations}")
        
        # Add warnings if any
        if self.current_model_info.warnings:
            warning_text = "; ".join(self.current_model_info.warnings)
            welcome_lines.append(f"⚠️ Warnings: {warning_text}")
    ...
    def _show_enhanced_error_message(self, error_message: str, error_analysis: dict):
        ...
        error_lines = [f"❌ Model loading failed: {error_message}"]
