...
from app.core.gpu_detector import GPUDetector, GPUBackend
...
class InstallationWorker(QThread):
        ...
        try:
            ...
            if not self.cancelled:
                if success:
                    ...
                    self.log_message.emit(f"✅ {self.backend.value.upper()} installation successful")
                else:
                    self.progress_updated.emit(f"{self.backend.value.upper()} installation failed")
                    self.log_message.emit(f"❌ {self.backend.value.upper()} installation failed")
    ...
    def cancel(self):
        ...
        self.log_message.emit("⚠️ Installation cancelled by user")
    ...
    def _detect_capabilities(self):
        ...
        try:
            ...
            if self.gpu_capabilities.has_gpu:
                ...
                self.detection_status_label.setText(f"✅ Found {gpu_count} GPU device(s)")
                self.detection_status_label.setStyleSheet("font-weight: bold; color: green;")
            else:
                self.detection_status_label.setText("⚠️ No GPU devices detected")
            ...
            self.detection_status_label.setText(f"❌ Detection failed: {str(e)}")
    ...
    def _verify_installation(self):
        ...
        try:
            ...
            if capabilities.has_gpu:
                ...
                self.verification_status.setText("✅ GPU acceleration is working correctly.")
                self.verification_status.setStyleSheet("color: green; font-weight: bold;")
            else:
                self.verification_results.append("\n⚠️ No GPU acceleration available.")
                self.verification_status.setText("⚠️ GPU acceleration is not available. Check installation and drivers.")
                self.verification_status.setStyleSheet("color: orange; font-weight: bold;")
            
        except Exception as e:
            self.verification_results.append(f"\n❌ Verification failed: {str(e)}")
            self.verification_status.setText("❌ Verification failed. Check installation.")
