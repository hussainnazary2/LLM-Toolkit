...
try:
    from app.services.gmail_client import GmailClient, GmailAuthError, GmailAPIError
    from app.services.gmail_oauth_errors import (
...
class EmailSettingsDialog(QDialog, DialogThemeMixin):
    ...
    def update_oauth_status(self, success: bool, message: str, email: str = ""):
        ...
        """
        if success:
            self.oauth_status_label.setText(f"✅ Authenticated as {email}" if email else "✅ Authenticated successfully")
    ...
    def _update_oauth_button_on_credentials_change(self):
        ...
        oauth_status = self.oauth_status_label.text()
        is_authenticated = "✅" in oauth_status and "authenticated" in oauth_status.lower()
    ...
    def _show_oauth_setup_help(self):
        ...
        help_content = """
        <h2>🔧 Gmail OAuth 2.0 Setup Guide</h2>
    ...
    def _generate_oauth_error_content(self, error_type: str, error_message: str, details: str = None) -> str:
        ...
        if error_type == "invalid_credentials":
            ...
            <h3>📋 Required Credentials Format</h3>
    ...
    def _on_save_clicked(self):
        ...
        if self.smtp_radio.isChecked():
            ...
            config = {
                ...
                "oauth_authenticated": "✅" in self.oauth_status_label.text(),
                "api_key": self.api_key_edit.text().strip()
            }
            ...
            if "Authenticated as " in status_text:
                # Extract email from "✅ Authenticated as user@gmail.com - Save button enabled"
    ...
    def _on_test_connection_clicked(self):
        ...
        if self.smtp_radio.isChecked():
            ...
            QMessageBox.warning(
                self,
                "⚠️ No Authentication Method Selected",
        ...
        try:
            ...
            try:
                ...
                if test_result.get('success', False):
                    ...
                    self.oauth_status_label.setText(f"✅ Connected as {email_address}")
                ...
                # Update OAuth status
                self.oauth_status_label.setText(f"❌ {e.get_user_friendly_message()}")
                self.oauth_status_label.setStyleSheet("color: #dc3545; font-size: 11px; margin-top: 5px;")
                ...
                if GMAIL_AVAILABLE and GmailOAuthErrorHandler:
                    ...
                    # Update OAuth status
                    self.oauth_status_label.setText(f"❌ {oauth_error.get_user_friendly_message()}")
                    self.oauth_status_label.setStyleSheet("color: #dc3545; font-size: 11px; margin-top: 5px;")
                else:
    ...
    def _show_comprehensive_oauth_error(self, oauth_error: 'GmailOAuthError'):
        ...
        # Error message
        error_label = QLabel(f"❌ {oauth_error.get_user_friendly_message()}")
    ...
    def _show_gmail_test_success(self, email_address: str, test_result: dict):
        ...
        message += f"🔐 Authentication: ✅ Valid OAuth 2.0 tokens\n"
        message += f"🌐 API Access: ✅ Gmail API accessible\n\n"
        message += f"Your Gmail OAuth configuration is working correctly and ready to use."
        
        QMessageBox.information(
            self,
            "✅ Gmail OAuth Test Successful",
            message
        )
    ...
    def _show_test_success(self, smtp_server: str, port: int, use_tls: bool):
        ...
        tls_status = "✅ TLS Enabled" if use_tls else "⚠️ TLS Disabled"
    ...
    def _show_gmail_test_success(self, email_address: str, test_result: dict):
        ...
        success_msg = f"""✅ Gmail OAuth Connection Successful!
...
You can now fetch emails, send messages, and manage your Gmail account securely."""
        ...
        self.oauth_status_label.setText(f"✅ Authenticated as {email_address}")
    ...
    def _show_gmail_test_error(self, error_title: str, error_details: str):
        ...
        error_msg = f"""❌ Gmail OAuth Connection Failed
...
Click 'Setup Help' for detailed troubleshooting guide."""
    ...
    def _perform_gmail_oauth_test(self, credentials_path: str):
        ...
        try:
            ...
            if test_result.get('success', False):
                ...
                # Update OAuth status
                self.oauth_status_label.setText(f"✅ Authenticated as {user_email}")
                self.oauth_status_label.setStyleSheet("color: #28a745; font-size: 11px; margin-top: 5px;")
    ...
    def set_oauth_authentication_status(self, success: bool, message: str, user_email: str = None):
        ...
        if success:
            # Show success status
            display_message = f"✅ Authenticated as {user_email}" if user_email else "✅ Authenticated"
            ...
            # Show error status
            self.oauth_status_label.setText("❌ Authentication failed")
            self.oauth_status_label.setStyleSheet("color: #dc3545; font-size: 11px; margin-top: 5px;")
